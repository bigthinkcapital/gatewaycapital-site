...
export default function ApplyForm() {
  ...
  const nextStep = () => {
    ...
    if (step === 2 && !form.fundingAmount) { setError('Please select a funding amount.'); return }
    ...
    return (
      <div className="bg-white border border-slate-200 rounded-2xl p-10 text-center">
        ...
        <p className="text-slate-500 text-base leading-relaxed mb-6 max-w-md mx-auto">
          Your application has been received. A dedicated funding specialist will contact you within the next few hours — often sooner. Check your email for confirmation.
        </p>
      ...
      <div className="p-8">
        ...
        {step === 1 && (
          <div>
            ...
            {/* Funding amount selection on step 1 for quick qualification feel */}
            <div className="mt-6">
              <p className="text-sm font-semibold text-slate-700 mb-3">How much are you looking for?</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {FUNDING_AMOUNTS.map(amt => (
                  <button
                    key={amt}
                    onClick={() => { set('fundingAmount', amt) }}
                    className={`text-left px-4 py-3 rounded-xl border text-sm font-medium transition-all ${
                      form.fundingAmount === amt
        ...
        {step === 2 && (
          <div>
            ...
            {/* Reinforce what they've selected */}
            {form.fundingAmount && (
              <div className="mt-5 p-4 bg-slate-50 rounded-xl border border-slate-200 text-sm text-slate-600">
        ...
        {step === 3 && (
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="brian@yourcompany.com"
                  className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                />
              </div>
              <div>
            ...
            {/* Summary */}
            <div className="mt-5 p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-1 text-sm text-slate-600">
            ...
            <p className="mt-4 text-xs text-slate-400 leading-relaxed">
              By submitting you agree to our Terms of Service and Privacy Policy. Submission does not constitute a binding agreement. A soft credit pull may occur for pre-qualification only.
            </p>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-100">
          {step > 1 ? (
            <button onClick={() => { setStep(s => s - 1); setError('') }} className="text-sm font-semibold text-slate-500 hover:text-slate-900 transition-colors flex items-center gap-1">
              ← Back
            </button>
          ) : (
            <div />
          ...
          {step < 3 ? (
            <button
              onClick={nextStep}
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm px-8 py-3 rounded-xl transition-all hover:shadow-lg hover:shadow-blue-600/25 hover:translate-x-0.5"
            >
              Continue →
            </button>
          ) : (
